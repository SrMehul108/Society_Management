...
const Societypopup = ({ isOpenDrop, togglePopup }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
      ...
      numWings: parseInt(formData.numWings, 10), 
      numFloors: parseInt(formData.numFloors, 10),
      flatsPerFloor: parseInt(formData.flatsPerFloor, 10),
    };

    try {
      const response = await CreateSociety(dataToSend); 
      if (response?.data?.status === 1) {
        togglePopup();
      } else {
        console.error("Failed to add society:", response?.data?.message);
      }
    } catch (error) {
      console.error("Error during society creation:", error);
    }
  };
                        ...
                        <div className="flex flex-wrap     space-y-4 sm:space-y-0 p-1 ">
                          <div className="flex flex-col w-full  sm:w-1/2 px-1">
                            ...
                            <input
                              type="number"
                              name="numFloors"
                              placeholder="How Many Floor per Flat"
                              className="border border-gray-300 p-2 rounded-md"
